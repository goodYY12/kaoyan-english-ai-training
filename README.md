...
- 已接入 2007-2026 年考研英语一数据入口。
- 阅读训练已覆盖 80 篇 Text 1-4，并保留完整语篇 + 5 题训练模式。
- 完形专项已覆盖 20 篇入口，支持 20 空选择、提交判分和错题保存。
- 翻译训练已接入 2007-2026 年入口；其中 2026、2025、2024、2019、2018 已补入参考译文或结构化训练数据。
- 写作训练已接入 2007-2026 年入口，部分年份仍需继续校对范文、表达和扣分点。

当前已知缺口：

- 2020、2017-2007 年翻译参考译文仍需继续补全和校对。
- 部分年份的阅读“干扰项分析 / 常见错因 / 出题人思维”还需要继续细化。
- 后续补题时必须来自可靠 PDF、官网/机构解析或用户提供资料；不能为了填满字段而编造内容。

更详细的数据补全记录见：[`docs/data-progress.md`](docs/data-progress.md)。
...
写作训练页提供“AI 辅助评阅”入口，英语一与英语二均可使用。

1. 先输入或粘贴作文；手写作文可额外上传清晰图片（不超过 2MB）。
2. 选择模型服务并填写自己的 API Key。内置 OpenAI、通义千问视觉模型、DeepSeek 和自定义 OpenAI 兼容接口。
3. API Key 不会写入源码、`localStorage` 或 GitHub；它只会随本次 HTTPS 请求发送给你选择的模型服务商。
4. 图片识别仅适用于支持视觉输入的模型；文本粘贴的识别和评阅更稳定。
5. 评分报告按英语一/英语二、大小作文的满分与字数要求生成，用于训练参考，不能代替官方人工阅卷。

本地用 `npm run dev` 启动时，Vite 不会运行 `api/evaluate-writing.js`。需要测试接口时，请使用已部署的 Vercel 网站，或安装 Vercel CLI 后执行 `vercel dev`。
