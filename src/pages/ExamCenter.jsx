...
function getModuleCards(year) {
  ...
  return [
    ...
    { title: "完形专项", to: "/cloze", status: clozeItems.length ? `${clozeItems[0].sourceType}数据` : "待补充" },
    { title: "翻译训练", to: "/translation", status: translation.status === "待补充" ? "待补充" : "已接入" },
    { title: "写作训练", to: "/writing", status: writing.status === "待补充" ? "待补充" : "已接入" },
    { title: "词汇复盘", to: "/vocabulary", status: readings.length || clozeItems.length ? "已接入" : "待补充" },
    { title: "错题复盘", to: "/mistakes", status: "本地记录" },
  ];
}

export default function ExamCenter() {
  const years = [...new Set([...getAvailableYears(), ...getClozeYears()])].sort(
    (a, b) => b - a,
  );
  ...
  return (
    <div>
      ...
      <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-500">
        年份从本地数据中自动读取。暂无数据的模块显示待补充，模拟数据会明确标注。
      </p>
